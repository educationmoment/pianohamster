...
fn meta(delta: u32, ty: u8, data: &[u8]) -> Vec<u8> {
    let mut v = vlq(delta);
    v.extend_from_slice(&[0xFF, ty, data.len() as u8]);
    v.extend_from_slice(data);
    v
}

fn text_event(delta: u32, ty: u8, s: &str) -> Vec<u8> {
    let b = s.as_bytes();
    let mut v = vlq(delta);
    v.extend_from_slice(&[0xFF, ty, b.len() as u8]);
...
fn note_on(delta: u32, ch: u8, note: u8, vel: u8) -> Vec<u8> {
    let mut v = vlq(delta);
    v.extend_from_slice(&[0x90 | (ch & 0x0F), note, vel]);
    v
}
fn note_off(delta: u32, ch: u8, note: u8, vel: u8) -> Vec<u8> {
    let mut v = vlq(delta);
    v.extend_from_slice(&[0x80 | (ch & 0x0F), note, vel]);
    v
}
fn program_change(delta: u32, ch: u8, prog: u8) -> Vec<u8> {
    let mut v = vlq(delta);
    v.extend_from_slice(&[0xC0 | (ch & 0x0F), prog]);
    v
}
fn control_change(delta: u32, ch: u8, cc: u8, val: u8) -> Vec<u8> {
    let mut v = vlq(delta);
    v.extend_from_slice(&[0xB0 | (ch & 0x0F), cc, val]);
    v
}
...
impl MidiBuilder {
    pub fn new(ppq: u16) -> Self {
        Self { ppq, conductor: Vec::new(), rh: Vec::new(), lh: Vec::new() }
    }
    ...
    pub fn time_sig(&mut self, num: u8, den: u8) {
        let mut pow = 0u8;
        let mut d = 1u8;
        while d < den && pow < 7 { d <<= 1; pow += 1; }
        self.conductor.extend_from_slice(&meta(0, 0x58, &[num, pow, 24, 8]));
    }

    pub fn key_sig(&mut self, sf: i8, minor: bool) {
        let b = (sf as i16).rem_euclid(256) as u8;
        self.conductor.extend_from_slice(&meta(0, 0x59, &[b, if minor { 1 } else { 0 }]));
    }

    pub fn program(&mut self, ch: u8, prog: u8) {
        self.rh.extend_from_slice(&program_change(0, ch, prog));
        self.lh.extend_from_slice(&program_change(0, ch, prog));
    }

    pub fn note(&mut self, right: bool, note: u8, dur_ticks: u32, vel: u8) {
        let trk = if right { &mut self.rh } else { &mut self.lh };
        trk.extend_from_slice(&note_on(0, 0, note, vel));
        trk.extend_from_slice(&note_off(dur_ticks, 0, note, 64));
    }

    pub fn rest(&mut self, right: bool, dur_ticks: u32) {
        let trk = if right { &mut self.rh } else { &mut self.lh };
        trk.extend_from_slice(&meta(dur_ticks, 0x01, &[]));
    }

    pub fn chord(&mut self, right: bool, notes: &[u8], dur_ticks: u32, vel: u8) {
        let trk = if right { &mut self.rh } else { &mut self.lh };
        for (i, n) in notes.iter().enumerate() {
            trk.extend_from_slice(&note_on(if i == 0 { 0 } else { 0 }, 0, *n, vel));
        }
        for (i, n) in notes.iter().enumerate() {
            trk.extend_from_slice(&note_off(if i == 0 { dur_ticks } else { 0 }, 0, *n, 64));
        }
    }

    pub fn pedal(&mut self, right: bool, down: bool, delta: u32) {
        let trk = if right { &mut self.rh } else { &mut self.lh };
        trk.extend_from_slice(&control_change(delta, 0, 64, if down { 127 } else { 0 }));
    }
}
